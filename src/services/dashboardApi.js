...
export const dashboardApi = api.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getVolumeOfUsageDetails: builder.query({
      query: () => ({
        url: `/api/dashboard/volumeUsage`,
        method: 'GET',
      }),
      ...
      async onQueryStarted(_, { dispatch, queryFulfilled }) {
        try {
          const { data } = await queryFulfilled; // already transformed
          dispatch(setVolumeOfUsageDetails(data));
        } catch (err) {
          // Prepare fallback dummy
          const dummyData = [
            { clientId: 1, name: 'Product A', value: 12 },
          ...
          dispatch(setVolumeOfUsageDetails(dummyData));
          dispatch(setError(err.message || 'Failed to fetch engagement details'));
        }
      },
    }),

    getRevenueAndProfitAtProductLevelDetails: builder.query({
      query: () => ({
        url: `/api/dashboard/revenueProfitProductLevel`,
        method: 'GET',
      }),
      ...
      async onQueryStarted(_, { dispatch, queryFulfilled }) {
        try {
          const { data } = await queryFulfilled; // already transformed
          dispatch(setRevenueAndProfitAtProductLevelDetails(data));
        } catch (err) {
          // Prepare fallback dummy
          const dummyData = [
            { clientId: 1, name: 'Product 1', revenue: 400000, profit: 24, category: 'one' },
          ...
          dispatch(setRevenueAndProfitAtProductLevelDetails(dummyData));
          dispatch(setError(err.message || 'Failed to fetch engagement details'));
        }
      },
    }),
...
export const {
  ...
  useGetVolumeOfUsageDetailsQuery,
} = dashboardApi;
